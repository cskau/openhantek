...
namespace Buudai {
	...
	Device::Device(QObject *parent) : QObject(parent) {
		...
		this->modelIds     <<  0x8102  <<  0x8312;
		this->modelStrings << "DDS120" << "DDS140";
	...
	QString Device::search() {
		if(this->error)
			return tr("Can't search for Buudai oscilloscopes: %1").arg(Helper::libUsbErrorString(this->error));
		...
		for(struct usb_bus *bus = usb_busses; bus; bus = bus->next) {
			for(device = bus->devices; device; device = device->next) {
				// Check VID and PID
				if(device->descriptor.idVendor == BUUDAI_DDS120_VID || device->descriptor.idVendor == BUUDAI_DDS140_VID) {
			...
			if(this->model >= 0) {
				...
				qDebug() << "libusb-0: found a Buudai scope at " << deviceAddress;
		...
		if(this->model >= 0) {
			...
			message = tr("No Buudai oscilloscope found");
#else
		...
		for(ssize_t deviceIterator = 0; deviceIterator < deviceCount; deviceIterator++) {
			...
			// Check VID and PID
			if ((this->descriptor.idVendor == BUUDAI_DDS120_VID) || (this->descriptor.idVendor == BUUDAI_DDS140_VID)) {
	...
	int Device::bulkWrite(unsigned char *data, unsigned int length, int attempts) {
		...
		for(int attempt = 0; (attempt < attempts || attempts == -1) && errorCode == LIBUSB_ERROR_TIMEOUT; attempt++)
			errorCode = usb_bulk_write(this->handle, BUUDAI_EP_OUT, (char *) data, length, BUUDAI_TIMEOUT);
		...
		return errorCode;
#else
		return this->bulkTransfer(BUUDAI_EP_OUT, data, length, attempts);
#endif
	}
	...
	int Device::bulkRead(unsigned char *data, unsigned int length, int attempts) {
		...
		errorCode = LIBUSB_ERROR_TIMEOUT;
		for(int attempt = 0; (attempt < attempts || attempts == -1) && errorCode == LIBUSB_ERROR_TIMEOUT; attempt++)
			errorCode = usb_bulk_read(this->handle, BUUDAI_EP_IN, (char *) data, length, BUUDAI_TIMEOUT);
		...
		return errorCode;
#else
		return this->bulkTransfer(BUUDAI_EP_IN, data, length, attempts);
#endif
	}
	...
	int Device::bulkReadMulti(unsigned char *data, unsigned int length, int attempts) {
		...
		for(packet = 0; received < length && errorCode == this->inPacketLength; packet++) {
			...
			errorCode = LIBUSB_ERROR_TIMEOUT;
			for(int attempt = 0; (attempt < attempts || attempts == -1) && errorCode == LIBUSB_ERROR_TIMEOUT; attempt++)
				errorCode = usb_bulk_read(this->handle, BUUDAI_EP_IN, (char *) data + packet * this->inPacketLength, qMin(length - received, (unsigned int) this->inPacketLength), BUUDAI_TIMEOUT);
#else
			errorCode = this->bulkTransfer(BUUDAI_EP_IN, data + packet * this->inPacketLength, qMin(length - received, (unsigned int) this->inPacketLength), attempts);
#endif
			if(errorCode > 0)
				received += errorCode;
		}
	...
	int Device::controlTransfer(unsigned char type, unsigned char request, unsigned char *data, unsigned int length, int value, int index, int attempts) {
		...
		for(int attempt = 0; (attempt < attempts || attempts == -1) && errorCode == LIBUSB_ERROR_TIMEOUT; attempt++)
#if LIBUSB_VERSION == 0
			errorCode = usb_control_msg(this->handle, type, request, value, index, (char *) data, length, BUUDAI_TIMEOUT);
#else
	...
	/// \return The #Model of the connected Buudai DSO.
